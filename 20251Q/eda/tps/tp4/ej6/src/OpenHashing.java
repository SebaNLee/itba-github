...
public class OpenHashing<K, V> implements IndexParametricService<K, V> {
	...
	private List<Slot<K, V>>[] lookup = (LinkedList<Slot<K, V>>[]) Array.newInstance(LinkedList.class,
			initialLookupSize);
	...
	public boolean remove(K key) {
		...
		if (lookup[hash(key)].size() == 0) // borro la estructura si no quedaron elementos en la lista
