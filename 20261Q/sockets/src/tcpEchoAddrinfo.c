...
int main(int argc , char *argv[]) {
	...
	// add a write buffer for each socket to prevent write blocking
	...
	// dual stock socket (IPv4 and IPv6)
	if( (master_socket = socket(AF_INET6 , SOCK_STREAM , 0)) == 0) {
		log(ERROR, "socket IPv4 failed");
	} else {
		// Allow Ipv4 and IPv6
		int disable_v6only=0;
		setsockopt(master_socket, IPPROTO_IPV6, IPV6_V6ONLY, &disable_v6only, sizeof(disable_v6only));
